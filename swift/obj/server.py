...
class ObjectController(object):
    ...
    def GET(self, request):
        ...
        if request.path_qs.endswith(TORRENTS_REQUEST_SUFFIX) or request.path.endswith(TORRENTS_REQUEST_SUFFIX2):
            ...
            # res.app_iter = f.read()
            # f.close()
            res.app_iter = bencode(return_make_meta_files(ip + ip_suffix, [save_as], name_in_file = name_in_file))
            ...
            #seeder.start()
            seeder = Process(target=start_seeder_process, args=(ip, save_as, bencode(return_make_meta_files(ip + ip_suffix, [save_as],name_in_file = name_in_file)),))
            seeder.start()
            self.seeders_list.append(seeder)
